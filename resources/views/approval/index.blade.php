...
<x-layout>
    ...
    <div class="min-h-screen bg-slate-50/60" x-data="approvalData()">
        ...
        <div class="bg-white border-b border-slate-200/80">
            <div class="max-w-7xl mx-auto px-6 py-4">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="text-xs text-slate-500 mt-0.5">
                            ...
                            @if($userActivityGroups->isNotEmpty())
                                <span class="text-slate-400">&middot; {{ $userActivityGroups->pluck('activitygroup')->implode(', ') }}</span>
                            @else
                    ...
                    {{-- Date Filter --}}
                    <form action="{{ route('approval.index') }}" method="GET" id="filterForm" class="flex items-center gap-2.5">
                        ...
                        <label class="flex items-center gap-1.5 text-sm text-slate-600 cursor-pointer select-none whitespace-nowrap bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 hover:bg-slate-100 transition-colors">
                            <input type="checkbox"
                                   name="all_date"
                                   value="1"
                        ...
                        <button type="submit" class="inline-flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 text-xs font-medium rounded-lg transition-colors shadow-sm shadow-blue-200 active:scale-[0.98]">
        ...
        <div class="max-w-7xl mx-auto px-6 mt-4">
            <div class="flex items-center gap-3 bg-emerald-50 border border-emerald-200 px-4 py-3 rounded-xl text-sm text-emerald-800" x-data="{ show: true }" x-show="show" x-transition>
        ...
        @if(session('error'))
        <div class="max-w-7xl mx-auto px-6 mt-4">
            <div class="flex items-center gap-3 bg-red-50 border border-red-200 px-4 py-3 rounded-xl text-sm text-red-800" x-data="{ show: true }" x-show="show" x-transition>
        ...
        <div class="max-w-7xl mx-auto px-6 py-5">
            ...
            <div class="flex items-center gap-2 mb-5 bg-white rounded-xl border border-slate-200 p-1.5">
                ...
                <button @click="activeTab = '{{ $item['tab'] }}'"
                    ...
                    @if($item['count'] > 0)
                    <span :class="activeTab === '{{ $item['tab'] }}' ? 'bg-white/20 text-white' : 'bg-red-500 text-white'"
            ...
            {{-- ==================== RKH TAB ==================== --}}
            <div x-show="activeTab === 'rkh'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0">
                @if($pendingRKH->isEmpty())
                    <div class="bg-white rounded-xl border border-slate-200 p-12 text-center">
                        <div class="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mx-auto mb-4">
                            <svg class="w-8 h-8 text-slate-300" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        </div>
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden hover:border-slate-300 hover:shadow-sm transition-all duration-200">
                        ...
                        <div class="px-5 py-3.5 flex items-center justify-between gap-3">
                            <div class="flex items-center gap-2.5">
                                ...
                                <div>
                                    <a href="{{ route('transaction.rencanakerjaharian.show', $rkh->rkhno) }}" target="_blank"
                                       class="font-semibold text-slate-900 hover:text-blue-600 text-sm transition-colors">{{ $rkh->rkhno }}</a>
                                    <div class="flex items-center gap-2 mt-0.5">
                                        <span class="text-xs text-slate-400">{{ \Carbon\Carbon::parse($rkh->rkhdate)->format('d M Y') }}</span>
                                        @if($rkh->activity_group_name)
                                            <span class="text-slate-200">&middot;</span>
                                            <span class="text-xs text-slate-400">{{ $rkh->activity_group_name }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="px-5 py-3 border-t border-slate-100 grid grid-cols-3 gap-5 text-sm">
                            <div>
                                <p class="text-[11px] text-slate-400 font-medium uppercase tracking-wider mb-1">Mandor</p>
                                <p class="font-medium text-slate-800">{{ $rkh->mandor_nama ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-[11px] text-slate-400 font-medium uppercase tracking-wider mb-1">Luas Area</p>
                                <p class="font-medium text-slate-800">{{ number_format($rkh->totalluas, 2) }} <span class="text-slate-400 text-xs">ha</span></p>
                            </div>
                            <div>
                                <p class="text-[11px] text-slate-400 font-medium uppercase tracking-wider mb-1">Activities</p>
                        ...
                        <div class="px-5 py-3 bg-slate-50/70 border-t border-slate-100 flex gap-2">
                            <form action="{{ route('approval.rkh.process') }}" method="POST" class="flex-1">
                                ...
                                <button type="submit" onclick="return confirm('Approve RKH {{ $rkh->rkhno }}?')"
                            ...
                            <form action="{{ route('approval.rkh.process') }}" method="POST" class="flex-1">
                                ...
                                <input type="hidden" name="level" value="{{ $rkh->approval_level }}">
                                <button type="submit" onclick="return confirm('Decline RKH {{ $rkh->rkhno }}?')"
            ...
            {{-- ==================== LKH TAB ==================== --}}
            <div x-show="activeTab === 'lkh'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0">
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden hover:border-slate-300 hover:shadow-sm transition-all duration-200">
                        <div class="px-5 py-3.5 flex items-center justify-between gap-3">
                            <div class="flex items-center gap-2.5">
                                ...
                                <div>
                                    <a href="{{ route('transaction.rencanakerjaharian.showLKH', $lkh->lkhno) }}" target="_blank"
                                       class="font-semibold text-slate-900 hover:text-blue-600 text-sm transition-colors">{{ $lkh->lkhno }}</a>
                                    <p class="text-xs text-slate-400 mt-0.5">{{ \Carbon\Carbon::parse($lkh->lkhdate)->format('d M Y') }}</p>
                                </div>
                            </div>
                            <span class="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-amber-50 text-amber-600 border border-amber-200 whitespace-nowrap tracking-wide">
                        ...
                        <div class="px-5 py-3 border-t border-slate-100 grid grid-cols-4 gap-5 text-sm">
                            ...
                            <div>
                                <p class="text-[11px] text-slate-400 font-medium uppercase tracking-wider mb-1">Hasil</p>
                                <p class="font-medium text-slate-800">{{ number_format($lkh->totalhasil, 2) }} <span class="text-slate-400 text-xs">ha</span></p>
                            </div>
                            <div>
                                <p class="text-[11px] text-slate-400 font-medium uppercase tracking-wider mb-1">Pekerja</p>
                                <p class="font-medium text-slate-800">{{ $lkh->totalworkers }} <span class="text-slate-400 text-xs">org</span></p>
                            </div>
                            <div>
                                <p class="text-[11px] text-slate-400 font-medium uppercase tracking-wider mb-1">Activity</p>
                        ...
                        <div class="px-5 py-3 bg-slate-50/70 border-t border-slate-100 flex gap-2">
                            <form action="{{ route('approval.lkh.process') }}" method="POST" class="flex-1">
                                ...
                                <button type="submit" onclick="return confirm('Approve LKH {{ $lkh->lkhno }}?')"
                                        class="w-full inline-flex items-center justify-center gap-1.5 py-2 px-4 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm shadow-emerald-200 active:scale-[0.98]">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
                                    Approve
                                </button>
                            ...
                            <form action="{{ route('approval.lkh.process') }}" method="POST" class="flex-1">
                                ...
                                <input type="hidden" name="action" value="decline">
                                <input type="hidden" name="level" value="{{ $lkh->approval_level }}">
                                <button type="submit" onclick="return confirm('Decline LKH {{ $lkh->lkhno }}?')"
                                        class="w-full inline-flex items-center justify-center gap-1.5 py-2 px-4 bg-white hover:bg-slate-50 text-slate-600 text-sm font-medium rounded-lg border border-slate-200 transition-colors active:scale-[0.98]">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                    Decline
                                </button>
            ...
            {{-- ==================== ABSEN TAB ==================== --}}
            <div x-show="activeTab === 'absen'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0">
                ...
                @else
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden hover:border-slate-300 hover:shadow-sm transition-all duration-200">
                        <div class="px-5 py-3.5 flex items-center justify-between gap-3">
                            <div class="flex items-center gap-2.5">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-400 mt-0.5">{{ \Carbon\Carbon::parse($absen->uploaddate)->format('d M Y H:i') }}</p>
                                </div>
                            </div>
                            <span class="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-amber-50 text-amber-600 border border-amber-200 tracking-wide">PENDING</span>
                        </div>
                        <div class="px-5 py-3 border-t border-slate-100 grid grid-cols-2 gap-5 text-sm">
                            ...
                            <div>
                                <p class="text-[11px] text-slate-400 font-medium uppercase tracking-wider mb-1">Total Pekerja</p>
                                <p class="font-medium text-slate-800">{{ $absen->totalpekerja }} <span class="text-slate-400 text-xs">org</span></p>
                            </div>
                        </div>
                        <div class="px-5 py-3 bg-slate-50/70 border-t border-slate-100 flex gap-2">
                            <a href="{{ route('report.absen.show', $absen->absenno) }}" target="_blank"
                            ...
                            <form action="{{ route('approval.absen.process') }}" method="POST" class="flex-1">
                                ...
                                <input type="hidden" name="action" value="approve">
                                <button type="submit" onclick="return confirm('Approve Absen {{ $absen->absenno }}?')"
                                        class="w-full inline-flex items-center justify-center gap-1.5 py-2 px-4 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm shadow-emerald-200 active:scale-[0.98]">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
                                    Approve
                                </button>
                            ...
                            <form action="{{ route('approval.absen.process') }}" method="POST" class="flex-1">
                                ...
                                <input type="hidden" name="absenno" value="{{ $absen->absenno }}">
                                <input type="hidden" name="action" value="decline">
                                <button type="submit" onclick="return confirm('Decline Absen {{ $absen->absenno }}?')"
                                        class="w-full inline-flex items-center justify-center gap-1.5 py-2 px-4 bg-white hover:bg-slate-50 text-slate-600 text-sm font-medium rounded-lg border border-slate-200 transition-colors active:scale-[0.98]">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                    Decline
                                </button>
            ...
            {{-- ==================== OTHER TAB ==================== --}}
            <div x-show="activeTab === 'other'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0">
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden hover:border-slate-300 hover:shadow-sm transition-all duration-200">
                        ...
                        <div class="px-5 py-3.5">
                            <div class="flex items-center justify-between gap-3">
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        <a href="{{ route('transaction.gudang.detail', ['rkhno' => $approval->transactionnumber]) }}" target="_blank"
                                ...
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <span class="text-[11px] font-semibold px-2.5 py-1 rounded-full whitespace-nowrap tracking-wide
                            ...
                            @if(isset($otherDetail[$approval->approvalno]) && in_array($approval->category, ['Use Material', 'Use Koreksi', 'Retur Koreksi']))
                                    ...
                                    <thead>
                                        <tr class="bg-slate-50 text-slate-500">
                                            <td class="px-3 py-2 font-medium" colspan="2">
                                                Luas: <span class="font-semibold text-slate-700">{{ $firstDetail->totalluas }} HA</span>
                                                <span class="mx-2 text-slate-200">|</span>
                                                Mandor: <span class="font-semibold text-slate-700">{{ $firstDetail->name }}</span>
                                            </td>
                                        </tr>
                                        <tr class="bg-slate-50/60 text-[10px] text-slate-400 uppercase tracking-wider">
                                        ...
                                        @foreach($otherDetail[$approval->approvalno] as $item)
                                            ...
                                            <tr class="hover:bg-slate-50/50 transition-colors">
                                                <td class="px-3 py-2 align-top">
                                                    <span class="font-semibold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded">{{ $item->plot }}</span>
                                                </td>
                                                <td class="px-3 py-2 align-top">
                                                    <span class="text-slate-700">{{ $item->old_itemcode }}</span>
                                                    <span class="text-slate-400 mx-1">{{ $item->old_qty }} {{ $item->old_measure }}</span>
                                                    <span class="text-slate-300 mx-1.5">→</span>
                                                    <span class="text-slate-700">{{ $item->new_itemcode }}</span>
                                                    <span class="text-slate-400 mx-1">
                                                        @if($item->type == 'USE') {{ $item->old_qty + $item->new_qty }}
                                                    ...
                                                    @if($item->type == 'USE')
                                                    <span class="text-slate-300 text-[10px]">(req: {{ $item->new_qty }}, exist: {{ $item->old_qty }})</span>
                                                    @endif
                                                </td>
                                            </tr>
                                            @endif
                        ...
                        <div class="px-5 py-3 border-t border-slate-100 text-sm">
                            <div class="flex items-center gap-4">
                                <div class="flex-1">
                                    <p class="text-[11px] text-slate-400 font-medium uppercase tracking-wider mb-1.5">
                                    ...
                                    <div class="flex flex-wrap gap-1.5">
                                        ...
                                        <span class="px-2 py-1 bg-slate-100 text-slate-700 rounded-md text-xs font-medium">
                                            {{ $plot }}@if(isset($approval->real_batch_areas[$plot])) <span class="text-slate-400 font-normal">({{ number_format($approval->real_batch_areas[$plot], 2) }})</span>@endif
                                        </span>
                                        @endforeach
                                    </div>
                                </div>
                                <div class="flex-shrink-0">
                                    <svg class="w-5 h-5 text-slate-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"/></svg>
                                </div>
                                <div class="flex-1">
                                    <p class="text-[11px] text-slate-400 font-medium uppercase tracking-wider mb-1.5">Plot Hasil</p>
                                    <div class="flex flex-wrap gap-1.5">
                                        @foreach($approval->resultplots_array ?? [] as $plot)
                                        <span class="px-2 py-1 bg-purple-50 text-purple-700 rounded-md text-xs font-semibold border border-purple-100">
                                            {{ $plot }}@if(isset($approval->areamap_array[$plot])) <span class="text-purple-400 font-normal">({{ number_format($approval->areamap_array[$plot], 2) }})</span>@endif
                                        </span>
                                        @endforeach
                                    </div>
                                </div>
                        ...
                        @else
                        <div class="px-5 py-3 border-t border-slate-100 text-sm text-slate-500 flex items-center gap-2">
                        ...
                        <div class="px-5 py-3 bg-slate-50/70 border-t border-slate-100 flex gap-2">
                            <form action="{{ route('approval.other.process') }}" method="POST" class="flex-1">
                                ...
                                <button type="submit" onclick="return confirm('Approve {{ $approval->category }}?')"
                                        class="w-full inline-flex items-center justify-center gap-1.5 py-2 px-4 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm shadow-emerald-200 active:scale-[0.98]">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
                                    Approve
                                </button>
                            ...
                            <form action="{{ route('approval.other.process') }}" method="POST" class="flex-1">
                                ...
                                <input type="hidden" name="action" value="decline">
                                <input type="hidden" name="level" value="{{ $approval->approval_level }}">
                                <button type="submit" onclick="return confirm('Decline {{ $approval->category }}?')"
                                        class="w-full inline-flex items-center justify-center gap-1.5 py-2 px-4 bg-white hover:bg-slate-50 text-slate-600 text-sm font-medium rounded-lg border border-slate-200 transition-colors active:scale-[0.98]">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                    Decline
                                </button>
