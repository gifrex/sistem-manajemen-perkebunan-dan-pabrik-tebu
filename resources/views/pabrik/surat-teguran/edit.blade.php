<x-layout>
    ...
    <div class="mx-auto py-4">
        ...
        <!-- Back -->
        <div class="mb-4">
            <a href="{{ route('pabrik.surat-teguran.show', $surat->id) }}"
        ...
        <div class="bg-white rounded-md shadow-md">
            <div class="px-6 py-4 border-b border-gray-200 flex items-center gap-3">
                <h2 class="text-base font-semibold text-gray-800">Edit Draft — {{ $surat->stgno }}</h2>
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">Draft</span>
            </div>

            <form method="POST" action="{{ route('pabrik.surat-teguran.update', $surat->id) }}"
                ...
                <div>
                    ...
                    <select name="targetcompany" id="targetcompany"
                        ...
                        <option value="{{ $company->companycode }}"
                            {{ old('targetcompany', $surat->targetcompany) === $company->companycode ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="jenisteguran" id="jenisteguran"
                        ...
                        <option value="REJECT" {{ old('jenisteguran', $surat->jenisteguran) === 'REJECT' ? 'selected' : '' }}>REJECT</option>
                        <option value="KOTOR" {{ old('jenisteguran', $surat->jenisteguran) === 'KOTOR' ? 'selected' : '' }}>KOTOR</option>
                    </select>
                    @error('jenisteguran')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="isiteguran" id="isiteguran" rows="6"
                        class="w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm px-3 py-2 @error('isiteguran') border-red-500 @enderror">{{ old('isiteguran', $surat->isiteguran) }}</textarea>
                    @error('isiteguran')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        Lampiran
                        <span class="text-gray-400 font-normal">(opsional — upload baru untuk mengganti)</span>
                    </label>
                    ...
                    <div class="mb-3">
                        ...
                        @if($ext === 'pdf')
                        <iframe src="{{ $lampiranUrl }}" class="w-full rounded-md border border-gray-200" style="height: 300px;"></iframe>
                        @else
                        <img src="{{ $lampiranUrl }}" alt="Lampiran saat ini" class="max-h-48 rounded-md border border-gray-200 shadow-sm">
                        @endif
                        <p class="mt-1 text-xs text-gray-500">Lampiran saat ini. Upload file baru di bawah untuk menggantinya.</p>
                    </div>
                    @endif
                    ...
                    <input type="file" name="lampiran" id="lampiran"
                        ...
                        class="block text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                    <p class="mt-1 text-xs text-gray-500">Format: JPEG, JPG, PNG, PDF &bull; Maks. 5 MB</p>
                    @error('lampiran')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                    <div x-show="fileName" x-transition class="mt-2 text-sm text-gray-600 flex items-center gap-1">
